...
uint8_t Hamster::rxPacket(uint8_t data)
{
  ...
  switch(state)
  {
    case 0:
      ...
      header_buf[header_index] = data;

      header_index = (header_index + 1) % 3;

      for (int i=0; i<3; i++)
      {
        index = (header_index + i) % 3;
        if (header_str[i] != header_buf[index])
        {
          header_rx = false;
          break;
        }
    ...
    case 1:
      rx_packet_buffer[rx_index++] = data;
